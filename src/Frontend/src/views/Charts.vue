  ...
  <div>
    ...
    <div class="chart-container">
      <div class="table-header">
        <div class="filters">
          <select v-model="selectedSensor" class="filter-select" @change="fetchChartData">
    ...
    <div class="sensor-grid" v-if="sensorTypes.length > 0">
      ...
      >
        ...
        <div style="height: 250px;">
          <Line :data="typeCharts[type] || emptyChart" :options="chartOptions" />
        </div>
      </div>
    </div>
  </div>
    ...
    const fetchChartData = async () => {
      ...
      try {
        ...
        if (readings.length > 0) {
          ...
          const color = colors[type] || { border: '#6366f1', background: 'rgba(99, 102, 241, 0.1)' }

          chartData.value = {
            labels: readings.map(r => new Date(r.timestamp).toLocaleTimeString('pl-PL')),
            datasets: [{
